...
let useWindowsAuth = (process.env.SQLSERVER_AUTH || '').toLowerCase() === 'windows' || !process.env.SQLSERVER_USER;
// If user contains domain (DOMAIN\user) treat as Windows auth
if (!useWindowsAuth && process.env.SQLSERVER_USER && (process.env.SQLSERVER_USER.indexOf('\\') !== -1 || process.env.SQLSERVER_USER.indexOf('/') !== -1)) {
  useWindowsAuth = true;
}
